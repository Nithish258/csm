...
export default function Invoices() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ clientId: '', amount: 0, dueDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], notes: '' });
    } catch (error: any) {
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
             ...
             <DialogContent className="max-w-md rounded-[3rem] p-10 bg-white dark:bg-slate-900 border-none shadow-2xl">
                <DialogHeader className="mb-8">
                   <DialogTitle className="text-2xl font-black uppercase tracking-tighter italic">{t('invoices.createTitle', 'Invoice Billing')}</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-8">
                   ...
                   <Button type="submit" disabled={loading} className="w-full h-16 bg-emerald-500 hover:bg-emerald-600 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-sm shadow-xl shadow-emerald-500/20">
                      {loading ? t('common.loading') : t('invoices.createButton', 'Commit Commercial Invoice')}
                   </Button>
                </form>
             </DialogContent>
          </Dialog>
                    ...
                    <TableRow key={inv.id} className="border-slate-100 dark:border-white/5 group hover:bg-slate-50 dark:hover:bg-white/5 transition-all">
                       ...
                       <TableCell className="px-10 py-6 text-right">
                          <div className="flex flex-col items-end">
                             ...
                             <span className="text-[8px] font-black uppercase text-slate-500 tracking-widest">Paid: ₹{inv.paidAmount.toLocaleString()}</span>
                          </div>
                       </TableCell>
                    </TableRow>
                 ))}
